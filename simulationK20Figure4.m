%Ergodic Rate, Monte Carlo Simulations compare with Analytical results-MMSE
...
for s=1:length(w)
    ...
    muRateMMSE(s)=log2(1+gamma_k_estimation);
end

plot(w,muRateMMSE,'r-','DisplayName','LIS subjects to the channel hardening effect (upper bound)');
grid on;
plot(w,ergodicR_k,'b-O','DisplayName','K=20, MMSE simulation');
%title('Ergodic Capacity of an LIS-based system with randomly located devices');
xlabel('Number of antennas on each subarray of LIS(M)');
